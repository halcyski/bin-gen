...
def require_tool(context: PlanningContext, capability: ToolCapability) -> Tool:
    tool = context.target.toolchain.tools.get(capability)
    if tool is None:
        raise PlanningError(
    ...
    def plan_sources(self, context: PlanningContext) -> StageResult:
        ...
        for source in context.target.sources:
            ...
            if not isinstance(adapter, GnuCcAdapter):
                raise PlanningError(f"{context.target.id}: capability {ToolCapability.COMPILE_C.value!r} "
                "requires gnu-cc compatible adapter, got {tool.interface.value!r}")
...
class LinkStage:
    def plan_product(
        ...
        tool = require_tool(context, ToolCapability.LINK)
        args = tool_args(context, ToolCapability.LINK)

        output = artifact_path(context, ArtifactFormat.ELF)
        objects = tuple(file.path for file in sources.files)
        
        adapter = ADAPTERS.get(tool.interface)
        if not isinstance(adapter, GnuCcAdapter):
            raise PlanningError(f"{context.target.id}: capability {ToolCapability.LINK.value!r} "
            "requires gnu-cc compatible adapter, got {tool.interface.value!r}")
        command = adapter.link(tool, objects, output, args)
